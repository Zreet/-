...
import static qqx5.XMLInfo.FireMaxNum;
...
class Calculate {
    ...
    /*void calculate(XMLInfo a) {
        ...
        System.out.println("basic用时 " + time(start,end));
        start = System.nanoTime();
        calcuSingle(a, false, true);// 非押爆，保存每个爆点分数、指数增加
        calcuSingle(a, false, false);// 押爆，保存每个爆点分数、指数增加
        calcuSingle(a, true, false);// 超极限，不保存，因为双排不会用到
        end = System.nanoTime();
        System.out.println("单用时 " + time(start,end));
        start = System.nanoTime();
        part();// 计算歌曲b段任意长度（应低于1-160）的分数指数
        end = System.nanoTime();
        System.out.println("简化计算用时 " + time(start,end));
        start = System.nanoTime();
        calcuDouble(a, true);// 非押爆，存气情况使用 part 简化计算
        calcuDouble(a, false);// 押爆，存气情况使用 part 简化计算
        end = System.nanoTime();
        System.out.println("双用时 " + time(start,end));
    }*/
